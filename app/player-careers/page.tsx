...
export default function PlayerCareersPage() {
  ...
  const filtered = useMemo(() => {
    ...
    return rows.filter((row) => {
      const matchesTab = row.section_code === activeTab;
      const name = `${row.player_name || ''} ${row.team_name || ''}`.toLowerCase();
      return matchesTab && (!q || name.includes(q));
    });
    ...
    <main className="min-h-screen bg-slate-950 text-white p-6">
      <div className="max-w-6xl mx-auto space-y-6">
        <header className="space-y-2">
          ...
          <h1 className="text-3xl font-bold">Oyuncu Kariyer Kartları</h1>
          <p className="text-slate-300">Resmi maçlar, özel/hazırlık maçları ve turnuvalar ayrı sekmelerde gösterilir.</p>
        </header>
        ...
        {!loading && !error && filtered.length === 0 && (
          <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 text-slate-300">
            Bu sekmede henüz veri yok. Maç istatistikleri girildikçe otomatik dolacak.
          </div>
        )}

        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {filtered.map((row) => (
            <section key={`${row.section_code}-${row.player_id}-${row.team_id || 'x'}`} className="bg-slate-900 border border-slate-800 rounded-2xl p-5 space-y-4">
              <div>
                <h2 className="text-xl font-bold">{row.player_name || `Oyuncu #${row.player_id}`}</h2>
                <p className="text-slate-400">{row.team_name || 'Takım bilgisi yok'} · {row.section_name}</p>
              </div>
